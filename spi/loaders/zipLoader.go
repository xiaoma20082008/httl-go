...
func (l *ZipLoader) Exists(name string, locale string, encoding string) bool {
	_, err := os.Stat(name)
	return err == nil
}

func (l *ZipLoader) Load(name string, locale string, encoding string) (httl.Resource, error) {
	if encoding == "" {
		encoding = l.encoding
	}
